...
public class MapsActivity extends AppCompatActivity
    ...
    private static final LatLng DEFAULT_LOCATION = new LatLng(43.0392, -76.1351);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button btnSetLoc = (Button) findViewById(R.id.btn_map_bottom);
        btnSetLoc.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (newMarker == null) {
                    ...
                    Intent intent = new Intent(getApplicationContext(), CreatePostActivity.class);      // create intent for data transfer
                    LatLng loc = newMarker.getPosition();

                    Bundle bun = new Bundle();                                                          // put lat, lng, addr into Bundle to transfer
                    bun.putDouble("lat", loc.latitude);
                    bun.putDouble("lng", loc.longitude);

                    Location temp = new Location(LocationManager.GPS_PROVIDER);                         // get address
                    temp.setLatitude(loc.latitude);
                    temp.setLongitude(loc.longitude);
                    String addr = getCurrentAddress(temp);

                    bun.putDouble("lat", loc.latitude);
                    bun.putDouble("lng", loc.longitude);
                    bun.putString("address", addr);
        ...
        MapFragment mapFragment = (MapFragment) getFragmentManager().findFragmentById(R.id.maptwo);
            ...
            if (hasFineLocationPermission == PackageManager.PERMISSION_DENIED) {
                ...
                myGoogleMap.setOnMapClickListener(new GoogleMap.OnMapClickListener() {
                    ...
                    public void onMapClick(LatLng latLng) {
                        if (newMarker == null) {
                            MarkerOptions options = new MarkerOptions().snippet("New Marker")
                                    .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_ORANGE))
                                    .anchor(0.5f, 0.5f);
                            options.position(latLng);
                            newMarker = myGoogleMap.addMarker(options);
                            newMarker.isDraggable();
                        }
                        else
